    ...
    public void RenderProjects() {
        ...
        for (int i = 0; i < Controller.instance.allProjects.ToArray().Length; i++) {
            if ((Controller.instance.allProjects[i].Level <= Controller.instance.GameLevel) &&
                ...
                g.transform.GetChild(8).GetComponent < TextMeshProUGUI > ().text = "Progress: " + ((int)(Controller.instance.allProjects[i].updateAndGetPercentage())).ToString() + "%";
                // if (Controller.instance.allProjects[i].PercentageDone > 0){
